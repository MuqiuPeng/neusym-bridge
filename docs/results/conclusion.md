...
| 操纵杆控制 | 离散杆操在连续域的能耗规划？ | **完成** | Greedy 86%，Relatum 56%，探索代价显著 |
...
| 规划器 | 成功率 | 平均能耗 | 平均步数 | 说明 |
|--------|--------|---------|---------|------|
| **Greedy** | **0.860** | **12.7** | **11.5** | 每步选 latent 距离最小的 lever |
| Relatum Explorer | 0.560 | 32.2 | 24.9 | 盲探索 + Relatum 坍缩 + Dijkstra |
| Random | 0.400 | 26.7 | 23.1 | 随机选 lever |

### 分析

1. **Greedy 胜出的原因**：它已知目标状态的 latent 表示，每步有明确的梯度方向；Relatum 探索器不使用目标 latent 信息，必须盲探索直到发现路径
2. **Relatum > Random**（56% vs 40%）：说明探索策略（优先未试 lever + frontier 导航）有效
3. **能耗差异**：Relatum 32.2 vs Greedy 12.7——探索本身消耗能量，这是离散迷宫实验中不存在的成本
4. **与迷宫的关键对比**：迷宫移动零成本 → 探索 100% 成功；触手每次探索有能量代价 → 探索预算有限
...
| 动作空间 | 4 方向 | 8 操纵杆 |
| 探索成本 | 零 | 每步 0-2.4 能耗 |
| 状态离散化 | 天然离散 | k-means (30 节点) |
| Relatum 成功率 | 100% | 56% |
| 主要限制 | 无 | 探索预算 + 离散化精度 |

**结论**：从离散迷宫到连续触手控制，探索代价从零变为非零，这是 Relatum 探索式规划性能下降的主因。在有目标信息的场景下，直接利用 latent 空间的 Greedy 更高效；Relatum 的优势在于**不需要目标的 latent 表示**——只需探索到目标节点即可触发坍缩。
...
### 8. 探索代价是从离散到连续的核心瓶颈（操纵杆控制）

操纵杆控制实验揭示了一个清晰的梯度：

| 环境 | 探索成本 | Relatum 成功率 | 对照成功率 |
|------|---------|---------------|-----------|
| 迷宫（离散，零成本） | 0 | 100% | Greedy 62% |
| 触手（连续，有成本） | 每步 0-2.4 | 56% | Greedy 86% |

迷宫中 Relatum 碾压 Greedy（100% vs 62%），因为探索免费且 Greedy 易陷死胡同。触手中 Greedy 反超（86% vs 56%），因为：1）它有 latent 目标方向，2）探索消耗能量预算。**Relatum 探索式规划的适用条件是探索成本低 + 目标信息不可用**；当目标 latent 已知且探索昂贵时，直接利用 latent 空间更高效。
