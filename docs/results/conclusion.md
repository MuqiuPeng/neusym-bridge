...
### 8. 图连通性（非离散化漂移）是连续域符号规划的核心瓶颈（操纵杆 + Exec-Aware）

操纵杆控制和 Exec-Aware 实验联合揭示了一个被逐步修正的诊断链：

| 实验 | 初始诊断 | 修正后诊断 |
|------|---------|-----------|
| 操纵杆（LeWM encoder） | 离散化漂移 → 多步偏航 | ✗ 特定于 LeWM 预训练 |
| Exec-Aware（新 encoder） | 新 encoder 应改善漂移 | ✗ 漂移已是 0%（100% 一致） |
| 最终诊断 | — | **图稀疏 → 不连通 → no-path 失败** |

| 环境 | 节点一致率 | 图边数 | no-path 率 | 成功率 |
|------|----------|--------|-----------|--------|
| 迷宫 | 100%（天然） | 完整 | 0% | 100% |
| 触手 (LeWM) | ~12% (LeWM 特有) | 463 | 8% | 12% |
| 触手（新 encoder） | **100%** | ~400 | **64-78%** | **0%** |

新 encoder 解决了漂移问题，但暴露了更根本的问题：30 节点 × 8 操纵杆的转移图在 3000 次探索后仍然大量不连通。**符号规划需要的不只是精确的离散化，还需要充分的图覆盖**。改进路径：增加探索量、减少节点数、或允许在线补充缺失边。
