...
| 操纵杆控制 | 离散杆操在连续域的能耗规划？ | **完成** | 两阶段评估：离散化漂移是主要瓶颈 |
...
**结论**：从迷宫到触手控制的性能悬崖不是 Relatum 推导的问题，而是**状态离散化的精度瓶颈**。迷宫是天然离散的——每步执行的结果和预期完全一致；连续域中 k-means 离散化导致多步规划累积漂移，使任何基于图的方法成功率都很低。改进方向：更细粒度的离散化（增大 k）、或在执行阶段加入 re-planning（每步重新定位当前节点并更新路径）。
...
### 8. 状态离散化精度是连续域符号规划的核心瓶颈（操纵杆控制）

操纵杆控制的两阶段评估（离线建图 + 在线执行）揭示：即使探索成本不计入评估，**所有基于离散图的规划器成功率都很低**（Relatum 12%，Greedy 12%）。原因不是选边策略，而是 **k-means 离散化导致的多步执行漂移**——每一步物理执行后 agent 落入的节点可能和图预测不同，多步累积后偏航严重。

| 环境 | 离散化 | 执行一致性 | Relatum 成功率 |
|------|--------|----------|---------------|
| 迷宫 | 天然离散（精确） | 100% | 100% |
| 触手 k=30 | k-means（有漂移） | ~12% | 12% |

Random walker 反而达 36%——因为它不依赖多步一致性，每步独立执行。

**核心认识**：符号规划要求状态离散化**可靠**——从 A 到 B 执行动作 X 的结果必须**可重现**地落在节点 B。连续域中达成这一点需要：更细的离散化、执行中 re-planning、或控制级反馈闭环。这是将迷宫实验中验证的符号推导能力**迁移到连续域**的核心工程挑战。
